...
function scheduleMeeting(startTime, durationMinutes) {
    ...
    let resBool;
    if (totalMinutesStartMeeting >= totalMinutesStart && totalMinutesEndMeeting <= totalMinutesEnd) {resBool = true}
    else resBool = false;

    return resBool;
}

let userInputStartMeeting = prompt("Введите время начала встречи в формате \"чч:мм\"");
let userInputDuration = Number(prompt("Введите длительность встречи в минутах"));
